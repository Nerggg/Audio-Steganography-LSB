...
const EmbedPanel: React.FC<EmbedPanelProps> = ({ onStatusUpdate, onCapacityUpdate, onEmbedComplete }) => {
  ...
  const handleEmbed = async () => {
    if (!coverAudio || !message.trim()) return

    setIsEmbedding(true)
    onStatusUpdate({
      isLoading: true,
      message: "Embedding message into audio...",
      type: "info",
    })
  ...
  const isEmbedReady = coverAudio && message.trim() && options.stegKey.trim()
    ...
    <div className="space-y-8">
      ...
      <div className="border border-cyan-400/50 rounded-lg p-6 bg-black/30 backdrop-blur-sm">
        ...
        <textarea
          ...
          className="w-full h-32 bg-black/50 border border-cyan-400/30 rounded-lg p-4 text-white font-mono resize-none focus:border-cyan-400 focus:outline-none focus:ring-2 focus:ring-cyan-400/25 transition-all"
          maxLength={1000}
        />

        <div className="flex justify-between items-center mt-2 text-sm">
          <span className="text-gray-400 font-mono">{message.length} / 1000 characters</span>
      ...
      <div className="border border-yellow-400/50 rounded-lg p-6 bg-yellow-900/10 backdrop-blur-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <label className="flex items-center gap-3 cursor-pointer">
            ...
            />
            <span className="text-white font-mono">AES-256 ENCRYPTION</span>
          </label>
